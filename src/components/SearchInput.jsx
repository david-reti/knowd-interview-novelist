...
*/
const SearchInput = ({search, searchResults}) => {
    ...
    const showSuggestions = searchResults.suggestions.length > 0 && 
    ...
    return (
        <label className="p-2 rounded-md bg-white shadow-xl w-5/6 md:w-3/5 max-w-[1280px]">
            ...
            <ul className={"flex flex-col h-fit overflow-y-auto transition-[max-height] duration-200 " + (showSuggestions ? "max-h-64 p-1" : "max-h-0")}>
            ...
            { !showSuggestions && searchValue && <ThreeDots stroke='grey' fill='grey' height={12.5} strokeOpacity={1} fillOpacity={1} className='mx-auto my-24'/> }
        </label>
    );
}
